...
/**
 * Provide configuration options for the askui UI Controller
 *
 * @param {number} display - You can choose on which display you want to excecute all tests.
 * 0 is your main monitor.
 * If you want to use your second monitor you can change
 * the value to `1` (`2` for your third monitor etc.).
 * @param {string} binaryVersion - Choose the version of the askui UI Controller Server.
 * @param {number} port - The port the askui UI Controller is running on.
 * @param {string} host - The host the askui UI Controller is running on.
 * @param {boolean} minimize - askui UI Controller will be started as minimized window.
 * @param {boolean} overWriteBinary - Download the provided Version of askui UI Controller.
 * If a version is already downloaded. This version will be overwritten
 * @param {LogLevels} logLevel - You can use different types of Log Level.
 * Options: "fatal", "error", "warn", "info", "debug", "trace", "silent", "verbose".
 * @param {string} logFilePath - It is possible to specify a path for your log files.
