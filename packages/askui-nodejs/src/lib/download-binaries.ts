...
function buildBinaryNotAvailbleError(binaryVersion: string): Error {
  return new Error(`It seems that the askui UI Controller version "${binaryVersion}" for your system "${platform()} ${os.arch}" is not availble, Please contact as at info@askui.com for more information`);
}
...
export function downloadServerBinaries(binaryVersion: string): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    logger.info(`Start downloading askui UI Controller version "${binaryVersion}"`);
