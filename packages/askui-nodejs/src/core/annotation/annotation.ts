...
export class Annotation {
  ...
  toHtml() {
    ...
    script.innerHTML = `
      ...
      el[0].setAttribute("detectedobjects", JSON.stringify(${JSON.stringify(this.data)}));
    `;
  ...
  static fromJson(json: AnnotationJson, resizeRatio = 1): Annotation {
    return new Annotation(
      json.image,
      json.data.map((data) => DetectedElement.fromJson(data, resizeRatio)),
    );
  }
