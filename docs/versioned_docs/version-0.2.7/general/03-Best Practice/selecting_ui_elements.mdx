...
Sometimes two words are so similar, e.g., 'text' and 'test', that the previous method is selecting the wrong text. In this cases the `withExactText('text')` method can be used.
```javascript
